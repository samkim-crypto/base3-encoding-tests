...
//! during encoding.
//!
//! These ternary digits are then packed into 64-bit integers (`u64`) for compact
//! storage. Since `3^40 < 2^64`, each `u64` can hold 40 ternary digits. This provides
//! a good balance between processing performance and minimizing wasted space.
//!
//! # Encoded Format
//!
...
//! 2.  **Data Payload (N * 8 bytes)**: A sequence of 8-byte chunks, where each
//!     chunk is the big-endian representation of a `u64` value containing the
//!     encoded data.
...
// Each u64 chunk can hold 40 base-3 symbols (3^40 < 2^64).
const BASE3_SYMBOL_PER_CHUNK: usize = 40;
const ENCODED_BYTES_PER_CHUNK: usize = 8; // std::mem::size_of::<u64>()

/// Encodes two `BitVec`s into a single `Vec<u8>`.
///
/// This function takes two bit vectors, `bit_vec_base` and `bit_vec_fallback`,
/// and combines them using a ternary encoding scheme packed into `u64` chunks.
///
/// # Errors
...
pub enum DecodeError {
    ...
    InvalidLengthPrefix,
    /// The data payload is not a multiple of the 8-byte block size, indicating
...
pub fn decode(bytes: &[u8]) -> Result<(BitVec<u8, Lsb0>, BitVec<u8, Lsb0>), DecodeError> {
    ...
    for block_bytes in data_bytes.chunks_exact(ENCODED_BYTES_PER_CHUNK) {
        ...
        let mut block_num = u64::from_le_bytes(block_arr);
...
mod tests {
    ...
    fn test_round_trip_small() {
        let (base, fallback) = create_test_data(10); // Smaller than one chunk
        let encoded = encode(&base, &fallback).unwrap();
        let (decoded_base, decoded_fallback) = decode(&encoded).unwrap();
        assert_eq!(base, decoded_base);
        assert_eq!(fallback, decoded_fallback);
    ...
    fn test_round_trip_exact_chunk() {
        let (base, fallback) = create_test_data(BASE3_SYMBOL_PER_CHUNK); // 40
        let encoded = encode(&base, &fallback).unwrap();
        let (decoded_base, decoded_fallback) = decode(&encoded).unwrap();
        assert_eq!(base, decoded_base);
        assert_eq!(fallback, decoded_fallback);
    ...
    fn test_round_trip_multi_chunk_partial() {
        // 1 full chunk (40) and one partial chunk of 5
        let (base, fallback) = create_test_data(45);
        let encoded = encode(&base, &fallback).unwrap();
        let (decoded_base, decoded_fallback) = decode(&encoded).unwrap();
        assert_eq!(base, decoded_base);
        assert_eq!(fallback, decoded_fallback);
    ...
    fn test_round_trip_multi_chunk_exact() {
        let (base, fallback) = create_test_data(80); // 2 full chunks
        let encoded = encode(&base, &fallback).unwrap();
        let (decoded_base, decoded_fallback) = decode(&encoded).unwrap();
        assert_eq!(base, decoded_base);
        assert_eq!(fallback, decoded_fallback);
